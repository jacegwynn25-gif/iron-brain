...

{
  ...
  assert.notEqual(recommendation.source, 'exercise_history');
  assert.notEqual(recommendation.evidenceSource, 'direct_history');
}

{
  const recommendation = nextSetRecommendation({
    currentSet: { ...baseSet, setId: 'next_set', weight: 225, reps: 5, prescribedWeight: 225, prescribedRPE: 8 },
    historySets: [],
    sessionSets: [
      { ...baseSet, setId: 'skipped_hard_set', weight: 225, reps: 5, rpe: 10, completed: true, skipped: true },
      { ...baseSet, setId: 'next_set', weight: 225, reps: 5, prescribedWeight: 225, prescribedRPE: 8, completed: false },
    ],
    readiness: { score: 80, modifier: 1 },
  });

  assert.equal(recommendation.target?.weight, 225);
  assert.equal(recommendation.target?.restSeconds, null);
  assert.notEqual(recommendation.source, 'session_fatigue');
  assert.doesNotMatch(recommendation.reason, /last set/i);
}

{
  const program: ProgramTemplate = {
    id: 'qa_skipped_history_program',
    name: 'QA Skipped History Program',
    weeks: [{
      weekNumber: 1,
      days: [{
        dayOfWeek: 'Mon',
