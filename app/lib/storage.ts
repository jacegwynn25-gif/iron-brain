...
export async function saveWorkout(
  ...
  let completedSetsSent = session.sets?.filter((set) => isPerformedSetLog(set, { allowWarmup: true })).length ?? 0;
  ...
  try {
    ...
    completedSetsSent = sessionToSave.sets?.filter((set) => isPerformedSetLog(set, { allowWarmup: true })).length ?? 0;
    ...
    if (user) {
      ...
      try {
        const completedSets = sessionToSave.sets?.filter((set) => isPerformedSetLog(set, { allowWarmup: true })) ?? [];
      ...
      if (sessionToSave.sets && sessionToSave.sets.length > 0) {
        ...
        const setPayloads = sessionToSave.sets.map((set, index): TablesInsert<'set_logs'> => {
          ...
          const completed = set.completed === true && !skipped;
...
async function runPostSaveAnalytics(
  ...
  try {
    const completedSets = sets.filter((s) => isPerformedSetLog(s));
...
function calculateWorkoutSummaryFromSets(sets: SetLog[]): {
  ...
  const completedSets = sets.filter((set) => isPerformedSetLog(set, { allowWarmup: true }));
        ...
        if (cleanSets.length > 0) {
          ...
          const setPayloads = cleanSets.map((set, index): TablesInsert<'set_logs'> => {
            ...
            const payload: TablesInsert<'set_logs'> = {
              ...
              prescribed_weight: set.prescribedWeight != null ? Number(set.prescribedWeight) : null,
              actual_weight: skipped || set.actualWeight == null ? null : Number(set.actualWeight),
              weight_unit: set.weightUnit === 'kg' ? 'kg' : 'lbs',
              actual_reps: skipped || set.actualReps == null ? null : Math.round(Number(set.actualReps)),
              actual_rpe: skipped || set.actualRPE == null ? null : Number(set.actualRPE),
              actual_rir: skipped || set.actualRIR == null ? null : Number(set.actualRIR),
...
export function getLastWorkoutForExercise(exerciseId: string): {
  ...
  const exerciseSets = lastSession.sets.filter(s => s.exerciseId === exerciseId && isPerformedSetLog(s));
